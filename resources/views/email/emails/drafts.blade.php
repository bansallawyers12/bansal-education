            ...
            <div class="min-h-0 flex-1 overflow-auto">
                @if($emails->isEmpty())
                    <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                        ...
                        <a href="{{ route('emails.compose') }}" class="mt-2 text-blue-400 hover:text-blue-300">Compose new email</a>
                    </div>
                @else
                            ...
                            @foreach($emails as $email)
                                <tr class="group transition-colors {{ $selected && $selected->id === $email->id ? 'bg-blue-600/10' : 'hover:bg-slate-800/50' }}">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 text-right text-sm sm:px-6">
                                        <a href="{{ route('emails.compose', ['draft' => $email->id]) }}" class="text-blue-400 hover:text-blue-300">Edit</a>
                                        <span class="mx-1.5 text-slate-600">|</span>
                                        <form action="{{ route('emails.outbound.destroy', $email) }}" method="POST" class="inline" onsubmit="return confirm('Move to Trash?');">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="relative flex min-h-0 flex-1 flex-col p-4 sm:p-6">
                    <div class="space-y-4 border-b border-slate-800 pb-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('emails.compose', ['draft' => $selected->id]) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-500">Edit draft</a>
                            <form action="{{ route('emails.outbound.destroy', $selected) }}" method="POST" class="inline" onsubmit="return confirm('Move to Trash?');">
                                @csrf
                                @method('DELETE')
