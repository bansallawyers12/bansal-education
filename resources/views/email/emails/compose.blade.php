...
@section('content')
    <div class="mx-auto max-w-4xl">
        <form action="{{ $draft ? route('emails.compose.update', $draft) : route('emails.compose.store') }}" method="POST" id="compose-form" class="space-y-6">
            @csrf
            ...
            <div class="flex flex-wrap gap-3 border-t border-slate-800 pt-6">
                ...
                <a href="{{ route('emails.index') }}" class="inline-flex items-center gap-2 rounded-lg border border-slate-600 px-4 py-2.5 text-sm font-medium text-slate-400 hover:bg-slate-800">
                    Cancel
                </a>
            </div>
        </form>
        ...
        @if($drafts->isNotEmpty())
            <div class="mt-10 rounded-xl border border-slate-800 bg-slate-900/50 p-4">
                ...
                <ul class="mt-2 space-y-1.5">
                    @foreach($drafts as $d)
                        <li>
                            <a href="{{ route('emails.compose', ['draft' => $d->id]) }}" class="text-sm text-blue-400 hover:text-blue-300">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('save-template-btn')?.addEventListener('click', async function() {
                ...
                try {
                    const r = await fetch('{{ route("emails.templates.store") }}', { method: 'POST', body: fd, headers: { 'X-Requested-With': 'XMLHttpRequest' } });
