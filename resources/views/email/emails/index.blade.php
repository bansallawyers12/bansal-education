...
@section('title', 'Inbound Emails')
@section('heading', 'Email Inbox')
    ...
    <div class="flex h-[calc(100vh-8rem)] flex-col gap-4 lg:flex-row lg:gap-6">
        ...
        <div class="flex min-h-0 flex-1 flex-col rounded-xl border border-slate-800 bg-slate-900/50 lg:max-w-2xl">
            <div class="shrink-0 border-b border-slate-800 px-4 py-3 sm:px-6">
                <h2 class="text-base font-semibold text-slate-100">Inbound Emails</h2>
                <p class="mt-0.5 text-sm text-slate-400">Recent incoming emails via SendGrid</p>
            </div>
            <div class="min-h-0 flex-1 overflow-auto">
                @if($emails->isEmpty())
                    <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                        ...
                        <p class="mt-4 text-slate-400">No emails yet</p>
                        <p class="mt-1 text-sm text-slate-500">Inbound emails will appear here when received.</p>
                    </div>
                @else
                    <table class="min-w-full divide-y divide-slate-800">
                        <thead class="sticky top-0 bg-slate-900/95 backdrop-blur">
                            <tr>
                                ...
                                <th class="hidden px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-400 lg:table-cell lg:px-6">Received</th>
                            ...
                            @foreach($emails as $email)
                                <tr class="group transition-colors {{ $selected && $selected->id === $email->id ? 'bg-blue-600/10' : 'hover:bg-slate-800/50' }}">
                                    <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-300 sm:px-6">
                                        <a href="{{ route('emails.index', ['id' => $email->id] + request()->query()) }}" class="block font-medium text-slate-200 hover:text-blue-400">
                                    ...
                                    <td class="hidden whitespace-nowrap px-4 py-3 text-sm text-slate-400 lg:table-cell lg:px-6">{{ $email->received_at?->format('M j, g:i A') ?? '—' }}</td>
                                    <td class="whitespace-nowrap px-4 py-3 sm:px-6">
                                        <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium {{ $email->status === 'processed' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400' }}">
                                            {{ ucfirst($email->status) }}
                                        </span>
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-3 text-right text-sm sm:px-6">
                                        <a href="{{ route('emails.index', ['id' => $email->id] + request()->query()) }}" class="text-blue-400 hover:text-blue-300">View</a>
                                        <span class="mx-1.5 text-slate-600">|</span>
                                        <a href="mailto:{{ $email->from_address }}?subject=Re: {{ rawurlencode($email->subject) }}" class="text-slate-400 hover:text-slate-300">Reply</a>
                                        <span class="mx-1.5 text-slate-600">|</span>
                                        <form action="{{ route('emails.inbound.destroy', $email) }}" method="POST" class="inline" onsubmit="return confirm('Move to Trash?');">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="flex min-h-[320px] flex-1 flex-col rounded-xl border border-slate-800 bg-slate-900/50 lg:min-w-0">
                ...
                <div class="relative flex min-h-0 flex-1 flex-col">
                    <div class="shrink-0 space-y-4 border-b border-slate-800 p-4 sm:p-6">
                        <div class="flex flex-wrap items-start justify-between gap-2">
                            ...
                            <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium {{ $selected->status === 'processed' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400' }}">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <a href="mailto:{{ $selected->from_address }}?subject=Re: {{ rawurlencode($selected->subject) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-blue-500">Reply</a>
                            <form action="{{ route('emails.inbound.destroy', $selected) }}" method="POST" class="inline" onsubmit="return confirm('Move to Trash?');">
                                @csrf
                                @method('DELETE')
                    ...
                    {{-- Subtle SendGrid logo in corner --}}
            ...
            @else
                <div class="flex flex-1 flex-col items-center justify-center p-8 text-center">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Click a row in the list to view the message here.</p>
                </div>
            @endif
        </div>
    </div>
