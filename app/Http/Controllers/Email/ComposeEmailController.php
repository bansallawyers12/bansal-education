...
class ComposeEmailController extends Controller
{
    ...
    public function update(Request $request, OutboundEmail $email): RedirectResponse|JsonResponse
    {
        if ($email->status !== OutboundEmail::STATUS_DRAFT) {
            return redirect()->route('emails.compose')->with('error', 'Cannot edit a non-draft email.');
        }
    ...
    private function handleAction(Request $request, OutboundEmail $email): RedirectResponse|JsonResponse
    {
        ...
        if ($action === 'send') {
            ...
            return redirect()->route('emails.outbox', ['id' => $email->id])
                ->with('success', 'Email moved to Outbox. Click "Send now" to send.');
        }

        return redirect()->route('emails.compose', ['draft' => $email->id])
            ->with('success', 'Draft saved.');
    }

    private function sendNow(OutboundEmail $email): RedirectResponse|\Illuminate\Http\JsonResponse
    {
        try {
            ...
            if (request()->wantsJson()) {
                return response()->json(['success' => true, 'redirect' => route('emails.sent', ['id' => $email->id])]);
            }

            return redirect()->route('emails.sent', ['id' => $email->id])
            ...
            if (str_contains(strtolower($e->getMessage()), 'authenticate') || str_contains(strtolower($e->getMessage()), '535')) {
                $hint = ' Check your SENDGRID_API_KEY in .env (ensure no typos, e.g. Mep1 not Mepl).';
    ...
    public function saveDraft(Request $request): RedirectResponse
    {
        ...
        if ($draftId) {
            $email = OutboundEmail::where('status', OutboundEmail::STATUS_DRAFT)->find($draftId);
            if ($email) {
                $email->update($validated);
                return redirect()->route('emails.compose', ['draft' => $email->id])
                    ->with('success', 'Draft saved.');
